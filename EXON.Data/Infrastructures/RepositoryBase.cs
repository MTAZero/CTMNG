...
namespace EXON.Data.Infrastructures
{
    public class RepositoryBase<T> : IRepository<T> where T : class
    {
        ...
        private RMDbContext dataContext;
        ...
        protected RMDbContext DbContext
        {
        ...
        public IEnumerable<T> GetByQueryString(string query)
        {
            return dataContext.Set<T>().SqlQuery(query);
        }

        public List<T> GetRandomList(List<T> listQuestion, int numQuestion)
        {
            if (listQuestion.Count < numQuestion)
                throw new Exception("Not Enough Question!");

            List<T> listRandomQuestion = new List<T>();

            List<ObjectForRandom> listRandom = new List<ObjectForRandom>();
            for (int q = 0; q < listQuestion.Count; q++)
            {
            ...
            while (numQuestionRandom < numQuestion)
            {
                int id = 0;
                do
                {
                    id = RM.Next(0, listRandom.Count);
                } while (listRandom[id].IsGeted);

                listRandom[id].IsGeted = true;
                numQuestionRandom++;
                listRandomQuestion.Add(listQuestion[listRandom[id].ObjectID]);
            }
